<template>
    <transition name="slide-fade-right" mode="in-out" v-if="visible">
        <div id="context_form" class="ui inverted" :style="{left: position.x + 'px', top: position.y+'px'}"
             v-window-draggable key="context_form">
            ...
            <div class="ui vertical inverted pointing menu" style="padding: 15px; background: rgb(31, 32, 33);margin:0">
                <form @submit.prevent="submit" id="ctx_form">
                    <div v-for="option in options">
                        <div class="ui inverted mini fluid labeled input input-transform right"
                             v-if="isType('string', option) && option.name != 'texture'" :key="option.name">
                        ...
                        <div class="ui inverted mini right fluid labeled input input-transform"
                             v-if="isType('number', option)" :key="option.name">
                            <div class="ui black label fixed noselect">
                                {{option.name | camelCaseToStr | capitalize}}
                            </div>
                            <input
                        ...
                        <div class="ui inverted mini fluid labeled input input-transform right"
                             v-if="option.name == 'texture'" style="overflow: inherit">
                            <div class="ui black label fixed noselect">
                                Texture
                            </div>
                            <div class="ui inverted tiny floating dropdown fluid button">
                                ...
                                <div class="inverted black menu">
                                    <a class="item" v-for="texture in textures" :value="texture.name"
                                       @click="option.value = texture.name">
            ...
            <div class="ui buttons fluid tiny">
                ...
                <button class="ui inverted blue button noradius" @click="submit" type="button" v-show="!loading">
                    Create
                </button>
                <button class="ui inverted blue loading button noradius" type="submit" form="ctx_form" v-show="loading">
                    Loading
                </button>
            </div>
        </div>
